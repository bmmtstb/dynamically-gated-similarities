...
from utils.state import B, DUMMY_BBOX, DUMMY_BBOX_BATCH, DUMMY_KP, DUMMY_KP_BATCH, J
...
class TestDynamicAlphaCombine(unittest.TestCase):
    ...
    default_cfg = fill_in_defaults(
        {
            "def_dynamic_alpha": {
                ...
                "softmax": False,
                "alpha_modules": ["box_fc1", "pose_fc1"],
            },
            "box_fc1": {
                "module_name": "FullyConnectedAlpha",
                "name": "bbox",
    ...
    s = State(bbox=DUMMY_BBOX_BATCH, keypoints=DUMMY_KP_BATCH)
    ...
    def test_dynamic_alpha_forward(self, device: Device):
        ...
        dac_const_sizes = DynamicAlphaCombine(
            config=fill_in_defaults(
                {"device": device, "def_dynamic_alpha": {"alpha_modules": ["box_fc1", "box_fc1"]}}, self.default_cfg
            ),
            path=["def_dynamic_alpha"],
        )
        self.assertEqual(len(dac_diff_sizes.alpha_model), self.S)
        self.assertEqual(len(dac_const_sizes.alpha_model), self.S)

        for model, s, tensors, result in [
            ("const", self.s, t.ones((self.S, self.D, self.T)), t.ones((self.D, self.T))),
            ("const", self.s, [t.ones((self.D, self.T)) for _ in range(self.S)], t.ones((self.D, self.T))),
            ("diff", self.s, t.ones((self.S, self.D, self.T)), t.ones((self.D, self.T))),
            ("diff", self.s, [t.ones((self.D, self.T)) for _ in range(self.S)], t.ones((self.D, self.T))),
        ]:
            with self.subTest(
                msg="device: {}, model: {}, type_si: {}, result: {}".format(device, model, type(tensors), result.shape)
            ):
                # send matrices to the respective device
                s = s.to(device=device)
                if isinstance(tensors, (list, tuple)):
                    tensors = [t_i.to(device=device) for t_i in tensors]
                else:
                ...
                if model == "const":
                    self.assertTrue(t.allclose(dac_const_sizes.forward(*tensors, s=s), result))
                elif model == "diff":
                    self.assertTrue(t.allclose(dac_diff_sizes.forward(*tensors, s=s), result))
                else:
                    self.fail("model undefined")
...
class TestDynamicAlphaCombineExceptions(unittest.TestCase):
    ...
    def setUp(self):
        self.model = DynamicAlphaCombine(config=self.default_cfg, path=["def_dynamic_alpha"])
    ...
    def test_type_error(self):
        ...
        with self.assertRaises(TypeError) as e:
            ...
            self.model.forward(*(1.0,), s=self.s)  # tensors should be tuple of t.Tensor
        ...
        with self.assertRaises(TypeError) as e:
            # noinspection PyTypeChecker
            self.model.forward(*self.dummy_t, s=1.0)  # alpha_inputs should be tensors
        self.assertIn("Expected state, but got", str(e.exception))

    def test_value_error_tensor_shapes(self):
        with self.assertRaises(ValueError) as e:
            self.model.forward(t.tensor([[1.0]]), t.tensor([[1.0, 2.0]]), s=self.s)  # Different shapes
    ...
    def test_runtime_error_tensor_device(self):
        if t.cuda.is_available():
            ...
            with self.assertRaises(RuntimeError) as e:
                self.model.forward(*tensors, s=self.s)  # Different devices
    ...
    def test_value_error_on_nof_tensors(self):
        ...
        with self.assertRaises(ValueError) as e:
            self.model.forward(*(t.ones((self.D, self.T)) for _ in range(self.S + 1)), s=self.s)
    ...
    def test_value_error_on_4d_tensors(self):
        with self.assertRaises(ValueError) as e:
            self.model.forward(*(t.ones((self.S, self.D, self.T)) for _ in range(self.S)), s=self.s)
        self.assertIn(f"Expected a 3D tensor, but got a tensor with shape", str(e.exception))

    def test_runtime_error_different_devices(self):
        if t.cuda.is_available():
            # tensor based
            with self.assertRaises(RuntimeError) as e:
                self.model.forward(*self.dummy_t_single.cpu(), s=self.s.to(device="cuda"))  # Different devices
            self.assertIn("The state should be on the same device as the tensors", str(e.exception))

    def test_batch_sizes_differ(self):
        single_s = State(bbox=DUMMY_BBOX, keypoints=DUMMY_KP)
        with self.assertRaises(ValueError) as e:
            _ = self.model.forward(*self.dummy_t, s=single_s)
        self.assertTrue("The batch size of the tensors (nof detections)" in str(e.exception), msg=e.exception)
