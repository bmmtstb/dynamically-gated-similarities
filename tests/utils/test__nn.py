...
class TestFullyConnected(unittest.TestCase):
    ...
    def test_set_up_hidden_layer_sizes_exceptions(self):
        for in_s, out_s, h_s in [
            (0, 1, [2, 3]),
            (1, 0, [2, 3]),
            (-1, 1, []),
            (5, 3, [-1, 2]),
        ]:
            with self.subTest(msg="in_s: {}, out_s: {}, h_s: {}".format(in_s, out_s, h_s)):
                with self.assertRaises(ValueError) as e:
                    _ = set_up_hidden_layer_sizes(input_size=in_s, output_size=out_s, hidden_sizes=h_s)
                self.assertTrue("Input, hidden or output size is <= 0." in str(e.exception), msg=e.exception)
