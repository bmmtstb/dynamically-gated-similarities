...
RCNN_DL_KEYS: dict[str, tuple[list[float], list[float], list[str]]] = {
    "dgs_pt21_rcnn_256x192_val": (
        [0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 0.99],  # [0.85]
        [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0],  # [0.4]
        [
            "iou",
            "OSNet",
            # "OSNetAIN",
...
if __name__ == "__main__":
    ...
    for DL_KEY, DGS_KEYS in (pbar_dl := tqdm(DL_KEYS.items(), desc="Dataloader", leave=False)):
        ...
        for DGS_KEY in (pbar_key := tqdm(DGS_KEYS, desc="similarities")):
    ...
    for RCNN_DL_KEY, SCORE_THRESHS, IOU_THRESHS, INITIAL_WEIGHT, DGS_KEYS in (
        pbar_dl := tqdm(RCNN_DL_KEYS, desc="RCNN Dataloaders", leave=False)
    ):
        pbar_dl.set_postfix_str(f"{RCNN_DL_KEY}_W{INITIAL_WEIGHT}")
